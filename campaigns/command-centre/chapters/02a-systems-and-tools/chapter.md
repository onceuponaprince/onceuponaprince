...
- [ ] **01c — ai-swarm extensions** — Carries Tier 3 from 01b: multi-round refinement, model routing per task, LAN bearer-token auth on workers, HTTP API to feed the 08+ webapp. Sits on the now-calmly-trustable 01b substrate; the HTTP API is functionally a precondition for the chapter climax.
